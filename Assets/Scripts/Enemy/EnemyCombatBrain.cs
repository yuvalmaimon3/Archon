...
public class EnemyCombatBrain : MonoBehaviour, IDeathHandler
{
    ...
    // Finds nearest live player via shared PlayerFinder utility.
    // Updates internal target reference and logs on change.
    private Transform FindNearestPlayer()
    {
        Transform nearest = PlayerFinder.FindNearest(transform.position, playerTag);
    ...
    private bool ExecuteAttack(AttackDefinition def)
    {
        ...
        switch (def.AttackType)
        {
            ...
            case AttackType.Summoning:
                int summoned = SummoningAttackExecutor.Execute(transform, def);
                return summoned > 0;
